<template>
  <div :class="[sticker,sticker+n]">
    ...
    <div :class="[notehead,notehead+n]">
     ...
     <div :class="[note,note+n]">{{ message }}</div>
...
export default {
  ...
  methods: {
    ...
    deleteSticker(){
      if(getCookie('username')==this.username){
        let params = new URLSearchParams()
        params.append('username', getCookie('username'))
        params.append('message', this.message)
        params.append('newdate',this.newdate)
        this.axios.post("http://127.0.0.1:5000/deleteMessage", params).then(res => {
            if (res.data== "deleted"){
              alert("删除了一条留言(/≧▽≦)/")
              this.$emit("delete")
            }else{
  ...
  props:['message','username','newdate'],
  mounted() {
    this.randomColor()
  }
};
